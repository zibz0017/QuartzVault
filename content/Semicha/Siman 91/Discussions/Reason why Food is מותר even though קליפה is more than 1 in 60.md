...
### Stream is Broken
#### בב״ח
1. If the stream is broken, then it depends: if we are talking about meat/dairy onto its opposite, then we have two points to be מקל:
	1. The meat/dairy needs to absorb into the food.
	2. The meat/dairy needs to cook the food (at least, to make it אסור דאורייתא, as meat and dairy needs to be cooked together to be אסור דאורייתא).
2. As such, in a case of הפסד מרובה, one can be מקל.
#### איסור
1. If it is איסור onto התר, the only discussion is if the איסור absorbed into the התר. It doesn't matter if anything is cooked.
2. As such, in such a case, the food will always be אסור.