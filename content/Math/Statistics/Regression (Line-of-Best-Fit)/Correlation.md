...
With the [[Z-Score#Formula for Converting Values Into Z-Scores|z-score formula written out]], it becomes this:$$r=\frac{1}{n-1}\left(\sum_{i=1}^{n}\left(\left(\frac{x_i-\overline{x}}{SD_X}\right)\left(\frac{y_i-\overline{y}}{SD_Y}\right)\right) \right)$$
...
#### Final Form
$$r=\frac{\sum_{i=1}^{n}(x_i-\overline{x})(y_i-\overline{y})}{\sqrt{\left(\sum_{j=1}^{n}{(x_j-\overline{x})^2}\right)\left(\sum_{j=1}^{n}{(y_j-\overline{y})^2}\right)}}$$
